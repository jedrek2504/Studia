...
public class SudokuBoardTest {
    ...
    @Test //checks if the board is generated in random manner(no board is the same)
    void fillBoard() {
        this.board.fillBoard();
        var boards = new ArrayList<String>(); //List that contains String representation of my 2d sudoku board
        for (var i = 0; i < 10; i++) {
            this.board.fillBoard();
    ...
    void isCorrect() {
        ...
        for (int row = 0; row < 9; row++) {
            for (int col = 0; col < 9; col++) {
                ...
                assertTrue(this.board.checkAllConditions(row, col, temp)); //checks if we put original value back to its place (is curenntly 0) would it be correct?
